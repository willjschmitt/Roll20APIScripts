...
import { getObjects } from "./filters";
...
(function() {
    ...
    on('chat:message', (msg: ChatEventData) => {
        ...
        const beyondCaller = getObj('player', msg.playerid);

        if (command !== 'beyond') {
            return;
        }
        let importData = '';
        if(args.length < 1) { sendHelpMenu(beyondCaller); return; }

        const rootConfig = new Configuration(state);
        const callerConfig = rootConfig.configForPlayer(beyondCaller.id);
        let initTiebreaker = callerConfig.initTieBreaker;
        let languageGrouping = callerConfig.languageGrouping;
        
        // If not set, we default to true even without a config reset.
        let spellTargetInAttacks = true;
        if (callerConfig.hasOwnProperty('spellTargetInAttacks')) {
            spellTargetInAttacks = callerConfig.spellTargetInAttacks;
        }
        ...
        for(let i = 0; i < args.length; i+=2) {
            ...
            switch(k) {
                case 'help':
                    sendHelpMenu(beyondCaller);
                    return;

                case 'reset':
                    rootConfig.clearPlayerConfig(beyondCaller.id);
                    rootConfig.setDefaults(true);
                    rootConfig.sendConfigMenu(beyondCaller);
                    return;
                ...
                case 'imports':
                    if (args.length > 0) {
                        ...
                        callerConfig.imports[key] = value;
                    }
        ...
        const characterInput = JSON.parse(json).character as BeyondDNDCharacterSheetJSON;

        sendChat(SCRIPT_NAME,
                 '<div style="'+style+'">Import of <b>' + characterInput.name + '</b> is starting.</div>',
                 null, {noarchive:true});


        // Remove characters with the same name if overwrite is enabled.
        if(callerConfig.overwrite) {
            const existingCharacters = findObjs({
                _type: "character",
                name: callerConfig.prefix + characterInput.name + callerConfig.suffix
        ...
        const roll20Character = CharacterSheet.createNew(characterInput, callerConfig, msg.playerId);

        // base class, if set
        if (characterInput.classes && (characterInput.classes.length > 0)) {
            roll20Character.setSingleAttributes('class': characterInput.classes[0].definition.name);
                'subclass': characterInput.classes[0].subclassDefinition == null ? '' : characterInput.classes[0].subclassDefinition.name,
                'base_level': characterInput.classes[0].level
            });
        }
        
        // Make Speed String
        let weightSpeeds = characterInput.race.weightSpeeds;
        ...
        let speedMods = getObjects(characterInput.modifiers, 'subType', 'speed');
        if(speedMods != null) {
        ...
        speedMods = getObjects(characterInput.modifiers, 'subType', 'innate-speed-flying');
        if(speedMods != null) {
            speedMods.forEach((speedMod) => {
        ...
        speedMods = getObjects(characterInput.modifiers, 'subType', 'innate-speed-swimming');
        if(speedMods != null) {
            speedMods.forEach((speedMod) => {
                if(speedMod.type == 'set' && speedMod.id.indexOf('spell') == -1) {
                    if(speedMod.value == null) speedMod.value = weightSpeeds.normal.walk;
        ...
        speedMods = getObjects(characterInput.modifiers, 'subType', 'innate-speed-climbing');
        if(speedMods != null) {
            speedMods.forEach((speedMod) => {
                if(speedMod.type == 'set' && speedMod.id.indexOf('spell') == -1) {
                    if(speedMod.value == null) speedMod.value = weightSpeeds.normal.walk;
        ...
        speedMods = getObjects(characterInput.modifiers, 'subType', 'unarmored-movement');
        if(speedMods != null) {
            speedMods.forEach((speedMod) => {
        ...
        speedMods = getObjects(characterInput.modifiers, 'subType', 'speed');
        if(speedMods != null) {
            speedMods.forEach((speedMod) => {
                if(speedMod.type == 'bonus') {
                    speedMod.value = isNaN(weightSpeeds.normal.walk + speedMod.value) ? 0 : speedMod.value;
        ...
        // Languages
        if(callerConfig.imports.languages) {
            let languages = getObjects(characterInput, 'type', 'language');
            if(languageGrouping) {
                let langs = [];
                if(languages != null) {
        ...
        if(callerConfig.imports.traits) {
            // Background Feature
            if(characterInput.background.definition != null) {
                let btrait = {
                    name: characterInput.background.definition.featureName,
                    description: replaceChars(characterInput.background.definition.featureDescription),
                    source: 'Background',
                    source_type: characterInput.background.definition.name
                };
            ...
            if(characterInput.background.customBackground != null) {
                if(characterInput.background.customBackground.featuresBackground != null) {
                    let btrait = {
                        name: characterInput.background.customBackground.featuresBackground.featureName,
                        description: replaceChars(characterInput.background.customBackground.featuresBackground.featureDescription),
                        source: 'Background',
                        source_type: characterInput.background.customBackground.name
                    };
            ...
            // Feats
            characterInput.feats.forEach((feat, fi) => {
                let t = {
            ...
            // Race Features
            if(characterInput.race.racialTraits != null) {
                let ti = 0;
                characterInput.race.racialTraits.forEach((trait) => {
                    ...
                    let t = {
                        ...
                        source: 'Race',
                        source_type: characterInput.race.fullName
                    };

                    let attrs = createRepeatingTrait(object, t, ti);
                    Object.assign(repeating_attributes, attrs);

                    let spells = getFeatureSpells(characterInput, trait.id, 'race');
        ...
        let total_level = 0;
        let monk_level = 0;
        if(callerConfig.imports.classes) {
            let multiclasses = {};
            characterInput.classes.forEach((current_class, i) => {
                ...
                if(callerConfig.imports.class_traits){
                    let ti = 0;
                    current_class.definition.classFeatures.forEach((trait) => {
                        ...
                        if(option_class_features.indexOf(trait.name) !== -1) {
                            ti = importClassOptions(repeating_attributes, trait, current_class, characterInput.options.class, ti);
                            return;
                        }
                        ...
                        let spells = getFeatureSpells(characterInput, trait.id, 'class');
                        spells.forEach((spell) => {
                            spell.spellCastingAbility = _ABILITIES[spell.spellCastingAbilityId];
                            class_spells.push(spell);
                        });
                        ...
                        if(trait.name == 'Metamagic') {
                            characterInput.choices.class.forEach((option) => {
                ...
                // Class Spells
                if(callerConfig.imports.class_spells){
                    for(let i in characterInput.classSpells) {
                        let spells = characterInput.classSpells[i];
                        if(characterInput.classSpells[i].characterClassId == current_class.id) {
                            characterInput.classSpells[i].spells.forEach((spell) => {
        ...
        let hasArmor = false;
        if(callerConfig.imports.inventory) {
            // accumulate unique fighting styles selected
            let fightingStylesSelected = new Set()
            let fightingStyles = getObjects(characterInput.classes, 'name', 'Fighting Style');
            fightingStyles.forEach((fS) => {
                let fsOpts = getObjects(characterInput.choices, 'componentId', fS.id);
            ...
            const inventory = characterInput.inventory;
            let prevAdded = [];
            if(inventory != null) {
                ...
                inventory.forEach((item, i) => {
                    ...
                    if(typeof item.definition.damage === 'object' && item.definition.type !== 'Ammunition') {
                        ...
                        let cv = getObjects(characterInput.characterValues, 'valueTypeId', item.entityTypeId);
                        cv.forEach((v) => {
                        ...
                        if(isFinesse && getTotalAbilityScore(characterInput, 2) > getTotalAbilityScore(characterInput, item.definition.attackType)) {
                            item.definition.attackType = 2;
                        }

                        // Hexblade's Weapon
                        let characterValues = getObjects(characterInput.characterValues, 'valueId', item.id);
                        characterValues.forEach((cv) => {
                            if(cv.typeId == 29 && getTotalAbilityScore(characterInput, 6) >= getTotalAbilityScore(characterInput, item.definition.attackType)) {
                        ...
                        if(item.definition.isMonkWeapon && monk_level > 0) {
                            ...
                            let str = getTotalAbilityScore(characterInput, 1);
                            let dex = getTotalAbilityScore(characterInput, 2);
                            if(dex > str) {
                                item.definition.attackType = 2;
                            }
                        }
                    ...
                    if(item.definition.hasOwnProperty('armorClass')) {
                        ...
                        if(["Light Armor", "Medium Armor", "Heavy Armor"].indexOf(item.definition.type) >= 0) {
                            ...
                            let aac = getObjects(characterInput, 'subType', 'armored-armor-class');
                            aac.forEach((aacb) => {
        ...
        createUnarmoredDefense(repeating_attributes, characterInput, total_level);

        if(characterInput.spells.race.length > 0) {
            let spells = characterInput.spells.race;
            spells.forEach((spell) => {
                spell.spellCastingAbility = _ABILITIES[spell.spellCastingAbilityId];
                class_spells.push(spell);
            });
        ...
        if (callerConfig.imports.proficiencies) {     
            for (let half_proficiency of getObjects(characterInput.modifiers, 'type', 'half-proficiency')) {
            ...
            for (let half_proficiency_round_up of getObjects(characterInput.modifiers, 'type', 'half-proficiency-round-up')) {
                updateProficiency(modifiers, half_proficiency_round_up, PROFICIENCY_HALF_ROUND_UP);
            }
            for (let proficiency of getObjects(characterInput.modifiers, 'type', 'proficiency')) {
                updateProficiency(modifiers, proficiency, PROFICIENCY_FULL);
            }
            for (let expertise of getObjects(characterInput, 'type', 'expertise')) {
            ...
            for (let cv of getObjects(characterInput.characterValues, 'typeId', 26)) {
                if(cv.value == 4) {
                    for (let obj of getObjects(characterInput, 'type', 'proficiency')) {
        ...
        if (callerConfig.imports.bonuses) {
            ...
            for (let bonus of getObjects(characterInput.modifiers, 'type', 'bonus', ['item'])) {
        ...
        let contacts = '',
            treasure = '',
            otherNotes = '';
        if(callerConfig.imports.notes){
            contacts += (characterInput.notes.allies) ? 'ALLIES:\n' + characterInput.notes.allies + '\n\n' : '';
            contacts += (characterInput.notes.organizations) ? 'ORGANIZATIONS:\n' + characterInput.notes.organizations + '\n\n' : '';
            contacts += (characterInput.notes.enemies) ? 'ENEMIES:\n' + characterInput.notes.enemies : '';

            treasure += (characterInput.notes.personalPossessions) ? 'PERSONAL POSSESSIONS:\n' + characterInput.notes.personalPossessions + '\n\n' : '';
            treasure += (characterInput.notes.otherHoldings) ? 'OTHER HOLDINGS:\n' + characterInput.notes.otherHoldings : '';

            otherNotes += (characterInput.notes.otherNotes) ? 'OTHER NOTES:\n' + characterInput.notes.otherNotes + '\n\n' : '';
            otherNotes += (characterInput.faith) ? 'FAITH: ' + characterInput.faith + '\n' : '';
            otherNotes += (characterInput.lifestyle) ? 'Lifestyle: ' + characterInput.lifestyle.name + ' with a ' + characterInput.lifestyle.cost + ' cost.' : '';
        }

        let background = '';
        if(characterInput.background.definition != null) background = characterInput.background.definition.name;
        if(background == '' && characterInput.background.customBackground.name != null) background = characterInput.background.customBackground.name;

        let other_attributes = {
            // Base Info
            'level': characterInput.classes[0].level + multiclass_level,
            'experience': characterInput.currentXp,
            'race': (characterInput.race.baseName || characterInput.race.fullName),
            'subrace': characterInput.race.subRaceShortName,
            'background': background,
            'speed': speed,
            'hp_temp': characterInput.temporaryHitPoints || '',
            'inspiration': (characterInput.inspiration) ? 'on' : 0,
            'alignment': characterInput.alignmentId == null ? '' : alignments[characterInput.alignmentId],

            // Bio Info
            'age': (characterInput.age || ''),
            'size': (characterInput.size || ''),
            'height': (characterInput.height || ''),
            'weight': (characterInput.weight || ''),
            'eyes': (characterInput.eyes || ''),
            'hair': (characterInput.hair || ''),
            'skin': (characterInput.skin || ''),
            'character_appearance': (characterInput.traits.appearance || ''),

            // Ability Scores
            'strength_base': getTotalAbilityScore(characterInput, 1),
            'dexterity_base': getTotalAbilityScore(characterInput, 2),
            'constitution_base': getTotalAbilityScore(characterInput, 3),
            'intelligence_base': getTotalAbilityScore(characterInput, 4),
            'wisdom_base': getTotalAbilityScore(characterInput, 5),
            'charisma_base': getTotalAbilityScore(characterInput, 6),

            // Traits
            'personality_traits': characterInput.traits.personalityTraits,
            'options-flag-personality': '0',
            'ideals': characterInput.traits.ideals,
            'options-flag-ideals': '0',
            'bonds': characterInput.traits.bonds,
            'options-flag-bonds': '0',
            'flaws': characterInput.traits.flaws,
            'options-flag-flaws': '0',

            // currencies
            'cp': characterInput.currencies.cp,
            'sp': characterInput.currencies.sp,
            'gp': characterInput.currencies.gp,
            'ep': characterInput.currencies.ep,
            'pp': characterInput.currencies.pp,

            // Notes/Bio
            'character_backstory': characterInput.notes.backstory,
            ...
            'initiative_style': calculateInitiativeStyle(characterInput),
        ...
        processItem(characterInput, items, single_attributes, repeating_attributes, total_level)
    });
    ...
    const processItem = (character, items, single_attributes, repeating_attributes, total_level) => {
        ...
        if (nextItem === undefined) {
            ...
            if(class_spells.length > 0 && callerConfig.imports.class_spells) {
